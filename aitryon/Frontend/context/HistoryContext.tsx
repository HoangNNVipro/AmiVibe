...
export const HistoryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const fetchHistory = async () => {
        ...
        try {
            const [vmRes, tryOnRes] = await Promise.all([
                fetch(buildApiUrl(API_BASE, '/api/virtual-models')),
                fetch(buildApiUrl(API_BASE, '/api/try-ons'))
            ]);
    ...
    const addVirtualModel = async (item: any) => {
        try {
            const response = await fetch(buildApiUrl(API_BASE, '/api/virtual-models'), {
                method: 'POST',
    ...
    const updateVirtualModel = async (id: string, item: any) => {
        try {
            const response = await fetch(buildApiUrl(API_BASE, `/api/virtual-models/${id}`), {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(item)
            });
    ...
    const deleteVirtualModel = async (id: string, type: string) => {
        try {
            const endpoint = type === 'virtual-model'
              ? buildApiUrl(API_BASE, '/api/virtual-models')
              : buildApiUrl(API_BASE, '/api/try-ons');
    ...
    const removeImageFromRecord = async (id: string, type: string, imageUrl: string) => {
        try {
            const endpoint = type === 'virtual-model'
              ? buildApiUrl(API_BASE, `/api/virtual-models/${id}/remove-image`)
              : buildApiUrl(API_BASE, `/api/try-ons/${id}/remove-image`);
    ...
    const addTryOn = async (item: any) => {
        try {
            const response = await fetch(buildApiUrl(API_BASE, '/api/try-ons'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(item)
            });
    ...
    const deleteTryOn = async (id: string) => {
        try {
            const response = await fetch(buildApiUrl(API_BASE, `/api/try-ons/${id}`), {
                method: 'DELETE'
            });
            if (!response.ok) {
                const errorData = await response.json();
