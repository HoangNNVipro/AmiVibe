...
const Users = ({ token }) => {
  ...
  const getStatusDisplayLabel = (status) => {
    ...
    if (status === 'Active') return t('dashboard_active');
    if (status === 'Suspended') return t('dashboard_suspended');
    return status;
  };
    ...
    <div className="bg-slate-50 min-h-screen p-4 sm:p-8 select-none font-sans text-slate-700 antialiased relative">
      ...
      <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-2">
        {toasts.map(toastItem => (
          <CustomToast 
            ...
            type={toastItem.type} 
            onClose={() => setToasts(prev => prev.filter(item => item.id !== toastItem.id))} 
          />
        ))}
      </div>
      ...
      <div className="max-w-[1500px] mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{t('user_management_title')}</h1>
            <p className="text-sm text-slate-500 mt-1">{t('user_management_subtitle')}</p>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('user_total_customers')}</p>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('user_active_users')}</p>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('user_suspended')}</p>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div ref={statusDropdownRef} className="relative">
              ...
              >
                <span className="font-semibold text-slate-600">{t('user_filter_status', { status: getStatusDisplayLabel(filterStatus) })}</span>
          ...
          {(searchQuery || filterStatus !== 'All') && (
            <div className="flex items-center justify-between border-t border-slate-100 pt-3 text-xs">
              <span className="text-slate-400">
                <Trans
                  i18nKey="user_found_matching_users"
        ...
        <div className="overflow-x-auto pb-4">
          {filteredUsers.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-20 flex flex-col items-center justify-center text-slate-400 shadow-sm w-full">
              ...
              <p className="text-sm font-semibold text-slate-600">{t('user_no_users_found')}</p>
              <p className="text-xs text-slate-400 mt-1">{t('user_no_users_hint')}</p>
              <button 
                onClick={() => { setSearchQuery(''); setFilterStatus('All'); }}
            ...
            <table className="w-full text-left border-separate border-spacing-y-2 sm:border-spacing-y-3">
              <thead>
                <tr className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                  <th className="pb-1 px-3 sm:px-6 w-[40%] sm:w-[45%]">{t('user_member')}</th>
                  <th className="pb-1 px-3 sm:px-6 hidden md:table-cell w-auto">{t('user_email_address')}</th>
                  <th className="pb-1 px-3 sm:px-6 text-center w-auto whitespace-nowrap">{t('user_status')}</th>
                  <th className="pb-1 px-3 sm:px-6 text-right w-auto whitespace-nowrap">{t('user_actions')}</th>
                </tr>
              </thead>
                  ...
                  <tr key={user._id} className="group transition-all duration-150">
                    ...
                    <td className="py-3 sm:py-4 px-3 sm:px-6 bg-white group-hover:bg-slate-50/80 border-y border-slate-100/80 text-center transition-colors">
                      ...
                      >
                        ...
                        {user.status === 'Active' ? t('dashboard_active') : t('dashboard_suspended')}
                      </span>
                    </td>
                    <td className="py-3 sm:py-4 px-3 sm:px-6 bg-white group-hover:bg-slate-50/80 rounded-r-2xl border-y border-r border-slate-100/80 text-right transition-colors">
                      <div className="flex items-center justify-end gap-1 sm:gap-1.5">
                        ...
                        <button 
                          ...
                          title={t('user_delete_user_title')}
                        >
                          <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {editUser && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <form onSubmit={handleSaveEdit} className="bg-white rounded-3xl max-w-md w-full shadow-2xl relative border border-slate-100 p-6 space-y-4">
            <div>
              <h2 className="text-lg font-bold text-slate-950">{t('user_edit_account_title')}</h2>
              <p className="text-xs text-slate-400 mt-0.5">{t('user_edit_account_subtitle')}</p>
            </div>
            <div className="flex items-center gap-4 bg-slate-50 p-3 rounded-2xl border border-slate-100">
              <img className="w-14 h-14 rounded-full border shadow-xs bg-white" src={getAvatar(editUser)} alt={editUser.name} />
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('user_member_profile')}</p>
                <p className="text-sm font-bold text-slate-700 mt-0.5">{editUser.name}</p>
              </div>
            </div>
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-1.5">{t('user_full_name')}</label>
            ...
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-1.5">{t('user_email_address')}</label>
            ...
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-1.5">{t('user_new_password_reset')}</label>
              <input type="password" placeholder={t('user_password_placeholder')} value={editUser.newPassword || ''} onChange={(e) => setEditUser(prev => ({ ...prev, newPassword: e.target.value }))} className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-800/10 focus:border-slate-800 text-xs transition-all" minLength="8" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-1.5">{t('user_status')}</label>
              <select value={editUser.status} onChange={(e) => setEditUser(prev => ({ ...prev, status: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-xs bg-white text-slate-600 font-semibold cursor-pointer focus:outline-none">
                ...
                <option value="Suspended">{t('dashboard_suspended')}</option>
              </select>
            </div>
            <div className="border-t border-slate-100 pt-4 flex justify-end gap-3">
              <button type="button" onClick={() => setEditUser(null)} className="px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-xl transition-colors cursor-pointer">{t('user_cancel')}</button>
              <button type="submit" className="px-5 py-2 text-xs font-semibold text-white bg-slate-950 hover:bg-slate-850 rounded-xl shadow-md transition-all cursor-pointer">{t('user_save_changes')}</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-slate-100 p-6 relative overflow-hidden">
            ...
            <div className="flex items-center gap-4 mt-2">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900">{t('user_delete_account_title')}</h3>
                <p className="text-xs text-slate-500">{t('user_delete_account_subtitle')}</p>
              </div>
            </div>
            <div className="my-5 bg-slate-50 p-4 rounded-2xl border border-slate-100">
              <p className="text-sm text-slate-700 font-medium">{t('user_delete_confirmation')}</p>
              <p className="text-xs text-rose-700 font-semibold mt-1.5 truncate">{deleteUser.name} ({deleteUser.email})</p>
            </div>
            <div className="flex items-center gap-3 justify-end mt-4">
              <button onClick={() => setDeleteUser(null)} className="px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer">{t('user_cancel')}</button>
              <button onClick={handleDeleteConfirm} className="px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-sm font-semibold shadow-md shadow-rose-600/10 active:scale-95 transition-all cursor-pointer">{t('user_confirm_delete')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {createUserModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <form onSubmit={(e) => handleCreateUserSubmit(e, { name: e.target[0].value, email: e.target[1].value, password: e.target[2].value, status: e.target[3].value })} className="bg-white rounded-3xl max-w-md w-full shadow-2xl relative border border-slate-100 p-6 space-y-4">
            <div>
              <h2 className="text-lg font-bold text-slate-950">{t('user_create_new_user_title')}</h2>
              <p className="text-xs text-slate-400 mt-0.5">{t('user_create_new_user_subtitle')}</p>
            </div>
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-1.5">{t('user_full_name')}</label>
              <input type="text" placeholder={t('user_enter_customer_name')} className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-800/10 focus:border-slate-800 text-xs transition-all" required />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-1.5">{t('user_email_address')}</label>
              <input type="email" placeholder={t('user_customer_email_placeholder')} className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-800/10 focus:border-slate-800 text-xs transition-all" required />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-1.5">Password</label>
              <input type="password" placeholder={t('user_password_placeholder_min')} className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-800/10 focus:border-slate-800 text-xs transition-all" minLength="8" required />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block mb-1.5">{t('user_status')}</label>
            ...
            <div className="border-t border-slate-100 pt-4 flex justify-end gap-3">
              <button type="button" onClick={() => setCreateUserModal(false)} className="px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-xl transition-colors cursor-pointer">{t('user_cancel')}</button>
              <button type="submit" className="px-5 py-2 text-xs font-semibold text-white bg-slate-950 hover:bg-slate-850 rounded-xl shadow-md transition-all cursor-pointer">{t('user_create_user')}</button>
            </div>
          </form>
        </div>
      )}
