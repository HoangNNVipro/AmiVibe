...
import { imageUrlToImageData, buildApiUrl } from '../lib/utils';
...
const App: React.FC = () => {
    ...
    const fetchInitialImageStatus = async () => {
      if (saveToProductTarget) {
        ...
        if (productIds.length > 0) {
          try {
            const response = await fetch(buildApiUrl(API_BASE, '/api/products/fetch-by-ids'), {
              method: 'POST',
  ...
  const confirmSaveToProducts = async () => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl(API_BASE, '/api/products/sync-images'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          imageUrl: saveToProductTarget.imageUrl,
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch(buildApiUrl(API_BASE, '/api/products'));
        if (res.ok) {
